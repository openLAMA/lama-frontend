...
const OrganizationsBeingTestedChart: React.FC = () => {
  ...
  const organizationsBeingTestedData = useSelector(
    (state: RootState) => state.charts.organizationsBeingTestedData,
  );

  useEffect(() => {
    if (organizationsBeingTestedData) {
      if (chart) {
        chart.data.labels = organizationsBeingTestedData.labels;
        chart.data.datasets = organizationsBeingTestedData.datasets;
        chart.update();
      }
    }
  ...
  useEffect(() => {
    if (getChartsStatus.success && chartRefCurrent && !chart) {
      const newChart = new Chart(chartRefCurrent, {
        type: 'bar',
        data: {
          labels: organizationsBeingTestedData.labels,
          datasets: organizationsBeingTestedData.datasets,
        },
        options: {
          aspectRatio: 1,
  ...
  return (
    <LoadingSwapComponent
      ...
      <div className="chart-container">
        <canvas id="organization-being-tested-chart" ref={chartRef} />
      </div>
    </LoadingSwapComponent>
  );
};

export default OrganizationsBeingTestedChart;
