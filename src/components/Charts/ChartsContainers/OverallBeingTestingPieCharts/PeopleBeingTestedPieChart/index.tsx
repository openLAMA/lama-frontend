...
const PeopleBeingTestedPieChart: React.FC = () => {
  ...
  const overallPeopleBeingTestedPieData = useSelector(
    (state: RootState) => state.charts.overallPeopleBeingTestedPieData,
  );

  useEffect(() => {
    if (overallPeopleBeingTestedPieData) {
      if (chart) {
        chart.data.labels = overallPeopleBeingTestedPieData.labels;
        chart.data.datasets = overallPeopleBeingTestedPieData.datasets;
        chart.update();
      }
    }
  ...
  useEffect(() => {
    if (getChartsStatus.success && chartRefCurrent && !chart) {
      const newChart = new Chart(chartRefCurrent, {
        type: 'pie',
        data: {
          labels: overallPeopleBeingTestedPieData.labels,
          datasets: overallPeopleBeingTestedPieData.datasets,
        },
        options: {
          aspectRatio: 1,
  ...
  return (
    <LoadingSwapComponent
      ...
      <div className="chart-container">
        <canvas id="people-being-tested-pie-chart" ref={chartRef} />
      </div>
    </LoadingSwapComponent>
  );
};

export default PeopleBeingTestedPieChart;
