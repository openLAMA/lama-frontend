...
const UnauthenticatedRoute: React.FC<UnauthenticatedRouteProps> = (
  ...
  const { viewName, redirectIfAuth, ...rest } = props;
  const Component = pages[viewName];

  const authToken = useSelector((state: RootState) => state.authData.token);
  const roleType = useSelector((state: RootState) => state.authData.roleType);

  if (redirectIfAuth && authToken) {
    return <Redirect to={`${roleType.toLowerCase()}`} />;
  }
